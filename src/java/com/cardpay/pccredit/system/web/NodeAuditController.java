...
public class NodeAuditController implements JRadConstants {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "browse_sxinfocfg.page")
	public AbstractModelAndView browse_sxinfocfg(HttpServletRequest request) {
		NodeAuditFilter filter = new NodeAuditFilter();
		filter.setRequest(request);
		filter.setProductId(Constant.SX_INFO);
		filter.setNodeType(NodeAuditTypeEnum.SXInfo.name());
		QueryResult<NodeAuditForm> firstresult = nodeAuditService.findProductsNodeAuditByFilter(filter);
		//不存在结束节点则创建个结束节点
		if(firstresult.getTotalCount()<1){
			NodeAuditForm nodeAudit = new NodeAuditForm();
			...
			nodeAudit.setModifiedBy(user.getId());
			nodeAudit.setModifiedTime(new Date());
			nodeAudit.setProductId(Constant.SX_INFO);
			nodeAudit.setIsstart("NO");
			nodeAudit.setIsend("YES");
			nodeAudit.setNodeName("结束");
			nodeAudit.setNodeType(NodeAuditTypeEnum.SXInfo.name());
			nodeAuditService.insertNodeAudit(nodeAudit);
		}
		QueryResult<NodeAuditForm> result = nodeAuditService.findProductsNodeAuditByFilter(filter);
		JRadPagedQueryResult<NodeAuditForm> pagedResult = new JRadPagedQueryResult<NodeAuditForm>(filter, result);
		JRadModelAndView mv = new JRadModelAndView("/system/nodeaudit/nodeaudit_config_browse", request);
		mv.addObject(PAGED_RESULT, pagedResult);
		mv.addObject("productId", Constant.SX_INFO);
		mv.addObject("nodeType", NodeAuditTypeEnum.SXInfo.name());
		return mv;
	}
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "browse_zainfomationcfg.page")
	public AbstractModelAndView browse_zainfomationcfg(HttpServletRequest request) {
		NodeAuditFilter filter = new NodeAuditFilter();
		filter.setRequest(request);
		filter.setProductId(Constant.ZA_INFO);
		filter.setNodeType(NodeAuditTypeEnum.ZAInfo.name());
		QueryResult<NodeAuditForm> firstresult = nodeAuditService.findProductsNodeAuditByFilter(filter);
		//不存在结束节点则创建个结束节点
		if(firstresult.getTotalCount()<1){
			NodeAuditForm nodeAudit = new NodeAuditForm();
			...
			nodeAudit.setModifiedBy(user.getId());
			nodeAudit.setModifiedTime(new Date());
			nodeAudit.setProductId(Constant.ZA_INFO);
			nodeAudit.setIsstart("NO");
			nodeAudit.setIsend("YES");
			nodeAudit.setNodeName("结束");
			nodeAudit.setNodeType(NodeAuditTypeEnum.ZAInfo.name());
			nodeAuditService.insertNodeAudit(nodeAudit);
		}
		QueryResult<NodeAuditForm> result = nodeAuditService.findProductsNodeAuditByFilter(filter);
		JRadPagedQueryResult<NodeAuditForm> pagedResult = new JRadPagedQueryResult<NodeAuditForm>(filter, result);
		JRadModelAndView mv = new JRadModelAndView("/system/nodeaudit/nodeaudit_config_browse", request);
		mv.addObject(PAGED_RESULT, pagedResult);
		mv.addObject("productId", Constant.ZA_INFO);
		mv.addObject("nodeType", NodeAuditTypeEnum.ZAInfo.name());
		return mv;
	}
