...
*/
@Controller
@RequestMapping("/intopieces/intopiecesdsh10level4/*")
@JRadModule("intopieces.intopiecesdsh10level4")
public class Intopiecesdsh10level4Control extends BaseController{
	...
	public static final  Logger logger = Logger.getLogger(Intopiecesdsh10level4Control.class);
	/**
	 * 一级支行行长进件页面
	 * 
	 * @param filter
	...
	public AbstractModelAndView browse(@ModelAttribute CustomerApplicationProcessFilter filter, HttpServletRequest request) throws SQLException {
		...
		filter.setNodeName(Constant.status_dsh_10_level4);
		...
		JRadModelAndView mv = new JRadModelAndView(
				"/intopieces/intopieces_wait/intopiecesApprove_dsh_10level4", request);
	...
	public AbstractModelAndView iframeApprove(HttpServletRequest request) {
		...
		if (StringUtils.isNotEmpty(customerInforId)) {
			...
			mv.addObject("operate", Constant.status_dsh_10_level4);
		}
		return mv;
	}
}
