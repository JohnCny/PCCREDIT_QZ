...
*/
@Controller
@RequestMapping("/intopieces/intopiecesdsh10level2/*")
@JRadModule("intopieces.intopiecesdsh10level2")
public class Intopiecesdsh10level2Control extends BaseController{
	...
	public static final  Logger logger = Logger.getLogger(Intopiecesdsh10level2Control.class);
	/**
	 * 一级支行行长进件页面
	 * 
	 * @param filter
	...
	public AbstractModelAndView browse(@ModelAttribute CustomerApplicationProcessFilter filter, HttpServletRequest request) throws SQLException {
		...
		filter.setNodeName(Constant.status_dsh_10_level2);
		...
		JRadModelAndView mv = new JRadModelAndView(
				"/intopieces/intopieces_wait/intopiecesApprove_dsh_10level2", request);
	...
	public AbstractModelAndView iframeApprove(HttpServletRequest request) {
		...
		if (StringUtils.isNotEmpty(customerInforId)) {
			...
			mv.addObject("operate", Constant.status_dsh_10_level2);
		}
		return mv;
	}
}
