...
public class SundsHelper{
	...
	public void add(String batchId,String batchName,List<QzApplnAttachmentDetail> details_ls,String docId) throws SunTransEngineException {
		logger.info("批次["+batchName+"] 批次号["+batchId+"] 开始上传-start upload!");
		ClientApi api = new ClientApi(ip, port, username, password);
		ECMDoc doc = new ECMDoc();
		doc.setObjName("WD_CASES");
		doc.setBatchID(docId);
		doc.setBusiAttribute(DocKey.BUSI_START_TIME, docId.substring(0,8));
		...
		doc.setBusiAttribute("CASEDATE", docId.substring(0,8));
		...
		for(int i=0;i<details_ls.size();i++){
			...
			doc.setFileAttribute("IMAGEDATE", docId.substring(0,8));
		...
		// 调用上传接口
		api.add(doc);
		logger.info("批次["+batchName+"] 批次号["+batchId+"] 上传成功-end upload!");
	}
	...
	public void update(String batchId,String batchName,List<QzApplnAttachmentDetail> details_ls,String docId) throws SunTransEngineException {
		logger.info("批次["+batchName+"] 批次号["+batchId+"] 开始单张更新-start single update!");
		ClientApi api = new ClientApi(ip, port, username, password);
		ECMDoc doc = new ECMDoc();
		doc.setBreakPointResume(false);
		doc.setBatchID(docId);
		doc.setObjName("WD_CASES");
		doc.setQueryTime(docId.substring(0,8));
		...
		// 调用更新接口
		api.update(doc);
		logger.info("批次["+batchName+"] 批次号["+batchId+"] 单张更新成功-end single update!");
	}
	...
	public void addMore(String batchId,String batchName,List<QzApplnAttachmentDetail> details_ls,String docId) throws SunTransEngineException {
		logger.info("批次["+batchName+"] 批次号["+batchId+"] 开始批量更新-start batch update!");
		ClientApi api = new ClientApi(ip, port, username, password);
		ECMDoc doc = new ECMDoc();
		doc.setBreakPointResume(false);
		doc.setBatchID(docId);
		...
		// 调用更新接口
		api.update(doc);
		logger.info("批次["+batchName+"] 批次号["+batchId+"] 批量更新成功-end batch update!");
	}
	...
	public Pic parseXml(String xmlStr,int page,int limit,HttpServletRequest request,String sessionDocId,String First_flag) throws DocumentException, SundsException{
	...
	public void del(String file_no,String doc_id,String batchId) throws SunTransEngineException {
		logger.info("批次号["+batchId+"] 文件号["+file_no+"] 开始单张删除-start single del!");
		ClientApi api = new ClientApi(ip, port, username, password);
		ECMDoc doc = new ECMDoc();
		doc.setBreakPointResume(false);
		doc.setBatchID(doc_id);
		doc.setObjName("WD_CASES");
		doc.setQueryTime(doc_id.substring(0,8));
		...
		// 调用更新接口
		api.update(doc);
		logger.info("批次号["+batchId+"] 文件号["+file_no+"] 单张删除成功-end single del!");
	}
	
	public void delBatch(String batchId,String docId) throws SunTransEngineException {
		logger.info("批次号["+batchId+"] 开始批次删除-start batch del!");
		ClientApi api = new ClientApi(ip, port, username, password);
		ECMDoc doc = new ECMDoc();
		// 必传信息， 设置批次号
		doc.setBatchID(docId);
		...
		// 调用删除接口
		api.del(doc);
		logger.info("批次号["+batchId+"] 批次删除成功-end batch del!");
	}
}
