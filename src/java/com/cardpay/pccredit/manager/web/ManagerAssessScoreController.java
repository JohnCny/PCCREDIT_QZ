 ...
 */
@Controller
@RequestMapping("/manager/assessscorerecordpg/*")
@JRadModule("manager.assessscorerecordpg")
public class ManagerAssessScoreController extends BaseController{
	...
	public AbstractModelAndView displayOrganization(@ModelAttribute ManagerAssessmentScoreFilter filter,
		...
		QueryResult<TyManagerAssessment> result = null;
		if(customerManagerId!=null && !customerManagerId.equals("")){
			result =  managerAssessmentScoreService.findManagerAssessmentScoreByFilterqz(filter);
		}else{
			if(forms.size()>0){
	...
	public AbstractModelAndView display(HttpServletRequest request) {
		...
		if (StringUtils.isNotEmpty(id)) {
			TyManagerAssessment managerAssessmentScoreForm = managerAssessmentScoreService.findManagerAssessmentScoreById1(id);
