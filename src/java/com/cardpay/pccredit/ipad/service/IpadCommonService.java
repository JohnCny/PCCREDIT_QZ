...
public class IpadCommonService {
	...
	SimpleDateFormat formatter14 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	public CompositeData ipadAddCustomerInfo(CompositeData BODY) throws ParseException, IpadException {
		...
		Date today = new Date();
		String client_no = BODY.getField("CLIENT_CODE").strValue();
		
		//增加判断身份证号码是否存在
		CustomerInfor tmp = customerInforservice.findCustomerInforByCardId(BODY.getField("GLOBAL_ID").strValue());
		if(StringUtils.isEmpty(client_no)){//新增
			if(tmp != null){
				throw new IpadException("身份证号码"+BODY.getField("GLOBAL_ID").strValue()+"已存在");
			}
		}else{
			if(tmp != null && !tmp.getId().equals(client_no)){
				throw new IpadException("身份证号码"+BODY.getField("GLOBAL_ID").strValue()+"已存在");
			}
		}
		...
		if(StringUtils.isEmpty(client_no)){//新增
			...
			ecif = ecifService.findEcifByCustomerId(client_no);
		}
		...
		ecif.setIssDate(StringUtils.isEmpty(BODY.getField("ISS_DATE").strValue())?today:formatter10.parse(BODY.getField("ISS_DATE").strValue()));
		...
		ecif.setOrgRegisteredDate(StringUtils.isEmpty(BODY.getField("ORG_REGISTERED_DATE").strValue())?today:formatter10.parse(BODY.getField("ORG_REGISTERED_DATE").strValue()));
		...
		CustomerInfor info = null;
		if(StringUtils.isEmpty(client_no)){
			info = new CustomerInfor();
		}
		else{
			info = customerInforservice.findCustomerInforById(client_no);
		}
		...
		//更新或写入
		if(ecif.getId() == null && info.getId() == null){
	...
	public CompositeData ipadQueryCustomerInfo(CompositeData BODY) {
		...
		ECIF ecif = ecifService.findEcifByCustomerId(BODY.getField("CLIENT_CODE").strValue());
		...
		CLIENT_CODE.setValue(BODY.getField("CLIENT_CODE").strValue());
		...
		ORG_REGISTERED_DATE.setValue(formatter10.format(ecif.getOrgRegisteredDate()));
		respBody.addField("ORG_REGISTERED_DATE", ORG_REGISTERED_DATE);
	...
	public CompositeData ipadQueryApplyInfoList(CompositeData BODY,String limit,String page) throws Exception {
		...
		if(!IpadConstant.QUERY_TYPE_03.equals(QUERY_TYPE)){
			if(APPROVE_STATUS.substring(2, 3).equals("1")){
				status += "'save',";
			}
			if(APPROVE_STATUS.substring(3, 4).equals("1")){
				status += "'audit',";
			}
			if(APPROVE_STATUS.substring(4, 5).equals("1")){
				status += "'refuse',";
			}
			if(APPROVE_STATUS.substring(5, 6).equals("1")){
				status += "'RETURNAPPROVE',";
			}
			if(APPROVE_STATUS.substring(6, 7).equals("1")){
				status += "'returnedToFirst',";
			}
			if(APPROVE_STATUS.substring(7, 8).equals("1")){
				status += "'approved',";
			}
			...
			if(!StringUtils.isEmpty(APPROVE_STATUS)){
				if(APPROVE_STATUS.substring(0, 2).equals("00")){
					//nothing
				}
				else{
					chl = APPROVE_STATUS.substring(0, 2).equals("10")?"0":"1";
				}
			}
		}
		...
		if(IpadConstant.QUERY_TYPE_01.equals(QUERY_TYPE)){//01-查询已进件贷款申请列表
			customerInfoIpad_ls = customerInforDao.ipadApplyInfoJjByUserId(QUERY_TYPE,USER_NO,CLIENT_NAME,global_id,chl,status,limit,page);
		}else if(IpadConstant.QUERY_TYPE_02.equals(QUERY_TYPE)){//02-查询贷款申请待处理补退件列表
			customerInfoIpad_ls = customerInforDao.ipadApplyInfoThByUserId(QUERY_TYPE,USER_NO,CLIENT_NAME,global_id,chl,status,limit,page);
		...
		for(CustomerApplyInfoIpad obj : customerInfoIpad_ls ){
			...
			Field APPLY_LIMIT_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
    ...
    public CompositeData ipadQueryDhjcist(CompositeData BODY,String limit,String page) throws Exception {
		...
		for(CustomerDhJcInfoIpad obj : CustomerDhJcInfoIpad_ls ){
			...
			if(StringUtils.isNotEmpty(obj.getCheckDescribe())){
				...
				}else if(obj.getTaskType().equals("5")){
					TASK_TYPE.setValue("每月电话回访");
				}else if(obj.getTaskType().equals("6")){
					TASK_TYPE.setValue("季度回访");
				}
			}
			...
			Field LOAN_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
			LOAN_AMT.setValue(obj.getLoanAmt());
			struct.addField("LOAN_AMT", LOAN_AMT);
			
			Field LOAN_BALANCE = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
    ...
    //ipad 微贷任务提醒
  	public CompositeData ipadWdtxList(CompositeData BODY) throws Exception {
		...
		LOAN_APPLY_BACK_NUM.setValue(customerDhJcTxInfoIpad.getLoanApplyBackNum());
	...
	public CompositeData ipadDqwdList(CompositeData BODY,String limit,String page) throws Exception {
		...
		for(CustomerDqwdtxInfoIpad obj : CustomerDhJcInfoIpad_ls ){
			...
			Field PRESENT_BALANCE = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
			...
			Field RECE_INTEREST = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	...
	public CompositeData ipadAddCustomerApplyInfo(CompositeData BODY) throws ParseException, IpadException {
	    ...
	    String INPUT_WARE_REG_NO  = BODY.getField("INPUT_WARE_REG_NO").strValue();//进件号
	    String OPERATION_TYPE  = BODY.getField("OPERATION_TYPE").strValue();//操作类型 1 新增 2删除
	    ...
	    if(OPERATION_TYPE.equals("1")){
		    ...
		    // find product
		 	ProductAttribute product = productService.findProductAttributeById(prodId);
		 	if(product == null){
			...
			za.setApplicationId(appInfo.getId());
	    	...
	    	//流程申请表
	    	com.cardpay.pccredit.intopieces.model.CustomerApplicationInfo cusAppInfo = intoPiecesService.findCusAppInforByAppId(INPUT_WARE_REG_NO);
			if(cusAppInfo.getStatus().equals("save") || cusAppInfo.getStatus().equals("returnedToFirst")){
				intoPiecesService.deleteApply(INPUT_WARE_REG_NO);
			}
			else{
				throw new IpadException("不能删除申请中的进件!");
			}
	    }
	...
	public CompositeData ipadApplyUpload(CompositeData BODY) throws ParseException, IpadException {
		...
		//进件表
		CustomerApplicationInfo appInfo  = commonDao.findObjectById(CustomerApplicationInfo.class, INPUT_WARE_REG_NO);
		if(appInfo == null){
			throw new IpadException("无此进件!");
		}
		...
		QzApplnYwsqb qz = null;
		qz = ywsqbService.findYwsqbByAppId(INPUT_WARE_REG_NO);
		if(qz == null){
			qz = new QzApplnYwsqb();
		}
		qz.setApplicationId(BODY.getField("INPUT_WARE_REG_NO").strValue());
		qz.setCustomerId(BODY.getField("CLIENT_CODE").strValue());
		...
		qz.setApplyAmount(BODY.getField("LOAN_AMT")==null?null:Double.toString(BODY.getField("LOAN_AMT").doubleValue()));
		...
		qz.setMonthRepay(BODY.getField("MONTH_REPAY")==null?null:Double.toString(BODY.getField("MONTH_REPAY").doubleValue()));
		qz.setGuntThing(BODY.getField("PLEDGE_NAME").strValue());
		qz.setGuntPeople(BODY.getField("GURTER_ACCT_NAME").strValue());
		qz.setYearIncome(BODY.getField("LICENSE_INCOME")==null?null:Double.toString(BODY.getField("LICENSE_INCOME").doubleValue()));
		qz.setProfitType(BODY.getField("PROFIT_TYPE").strValue());
		qz.setProfit(BODY.getField("PROFIT_AMT")==null?null:Double.toString(BODY.getField("PROFIT_AMT").doubleValue()));
		qz.setTotalAssets(BODY.getField("ASSET_TOTAL_AMT")==null?null:Double.toString(BODY.getField("ASSET_TOTAL_AMT").doubleValue()));
		qz.setInMoney(BODY.getField("OUGHT_ACCEPT_AMT")==null?null:Double.toString(BODY.getField("OUGHT_ACCEPT_AMT").doubleValue()));
		qz.setOutMoney(BODY.getField("OUGHT_PAY_AMT")==null?null:Double.toString(BODY.getField("OUGHT_PAY_AMT").doubleValue()));
		qz.setOtherOut(BODY.getField("LIBILITY")==null?null:Double.toString(BODY.getField("LIBILITY").doubleValue()));
		qz.setMonthOtherIncome(BODY.getField("OTHER_INCOME")==null?null:Double.toString(BODY.getField("OTHER_INCOME").doubleValue()));
		qz.setOtherIncomeSrc(BODY.getField("OTHER_SOURCE").strValue());

		//经营信息
		QzApplnJyxx jyxx = null;
		jyxx = jyxxService.findJyxx(BODY.getField("CLIENT_CODE").strValue(),null);
		if(jyxx == null){
			jyxx = new QzApplnJyxx();
		}
		jyxx.setApplicationId(BODY.getField("INPUT_WARE_REG_NO").strValue());
		jyxx.setCustomerId(BODY.getField("CLIENT_CODE").strValue());
		...
		jyxx.setBussEmployeeNum(BODY.getField("EMPLOYEE_NUMBER")==null?null:Integer.toString(BODY.getField("EMPLOYEE_NUMBER").intValue()));
		...
		for(int i =0;i<gy.size();i++){
			...
			gys.setRate(gy.getStruct(i).getField("LOT")==null?null:Double.toString(gy.getStruct(i).getField("LOT").doubleValue()));
		...
		for(int i =0;i<kh.size();i++){
			...
			khs.setName(kh.getStruct(i).getField("CLIENT_NAME").strValue());
			khs.setRate(kh.getStruct(i).getField("LOT")==null?null:Double.toString(kh.getStruct(i).getField("LOT").doubleValue()));
			khs.setCondition(kh.getStruct(i).getField("DRAW_TYPE").strValue());
			khs.setLsh(i+1);
			khList.add(khs);
		}
		
		//货款标志LOAN_FLAG 
		qz.setBorrowHistory(BODY.getField("LOAN_FLAG").strValue());
		
		//贷款信息明细数组
		Array mx = BODY.getArray("LOAN_MSG_INFO_ARRAY");
		List<QzApplnYwsqbJkjl> jkjlList = new ArrayList<QzApplnYwsqbJkjl>();
		if(BODY.getField("LOAN_FLAG").strValue().equals("1")){
			for(int i =0;i<mx.size();i++){
				QzApplnYwsqbJkjl jkjl = new QzApplnYwsqbJkjl();
				jkjl.setBankOrOtherType(mx.getStruct(i).getField("LOAN_SOURCE").strValue());
				jkjl.setPurpose(mx.getStruct(i).getField("LOAN_PURPUSE_CODE").strValue());
				//TODO xiegl
				jkjl.setTotalAmount(mx.getStruct(i).getField("AMT").strValue());
				jkjl.setLoanDate(StringUtils.isEmpty(mx.getStruct(i).getField("DRAW_DOWN_DATE").strValue())?new Date():formatter10.parse(mx.getStruct(i).getField("DRAW_DOWN_DATE").strValue()));
				jkjl.setDeadline(mx.getStruct(i).getField("TERM").strValue());
				//TODO xiegl
				jkjl.setRates(mx.getStruct(i).getField("INT_RATE").strValue());
				jkjl.setRepayType(mx.getStruct(i).getField("REPAY_TYPE").strValue());
				jkjl.setGuaranteeMode(mx.getStruct(i).getField("GUARANTEE_MODE").strValue());
				//TODO xiegl
				jkjl.setRemainSum(mx.getStruct(i).getField("LOAN_BALANCE").strValue());
				jkjl.setLsh(i+1);
				jkjlList.add(jkjl);
			}
		}
		...
		qz.setHelpManagerName(BODY.getField("HELPERS").strValue());
		...
		//担保人信息数组
		String sql = "delete from qz_appln_dbrxx_dkjl where dbrxx_id in (select id from qz_appln_dbrxx where APPLICATION_ID = '"+INPUT_WARE_REG_NO+"')";
		commonDao.queryBySql(sql, null);
		sql = "delete from qz_appln_dbrxx_fc where dbrxx_id in (select id from qz_appln_dbrxx where APPLICATION_ID = '"+INPUT_WARE_REG_NO+"')";
		commonDao.queryBySql(sql, null);
		sql = "delete from qz_appln_dbrxx_jdc where dbrxx_id in (select id from qz_appln_dbrxx where APPLICATION_ID = '"+INPUT_WARE_REG_NO+"')";
		commonDao.queryBySql(sql, null);
		sql = "delete from qz_appln_dbrxx where APPLICATION_ID = '"+INPUT_WARE_REG_NO+"'";
		commonDao.queryBySql(sql, null);
		...
		for(int i =0;i<dbrarray.size();i++){
			...
			String SEQU_NO = dbrarray.getStruct(i).getField("SEQU_NO").strValue();
			dbrxx.setCustomerId(BODY.getField("CLIENT_CODE").strValue());
			dbrxx.setApplicationId(BODY.getField("INPUT_WARE_REG_NO").strValue());
			dbrxx.setName(dbrarray.getStruct(i).getField("GUARANTEE_NAME").strValue());
			dbrxx.setSex(dbrarray.getStruct(i).getField("SEX").strValue());
			dbrxx.setBirthday(StringUtils.isEmpty(dbrarray.getStruct(i).getField("BIRTH_DATE").strValue())?new Date():formatter10.parse(dbrarray.getStruct(i).getField("BIRTH_DATE").strValue()));
			dbrxx.setYeah(dbrarray.getStruct(i).getField("RESIDENT_YEAR").strValue());
			dbrxx.setGlobalType(dbrarray.getStruct(i).getField("GLOBAL_TYPE").strValue());
			dbrxx.setGlobalId(dbrarray.getStruct(i).getField("GLOBAL_ID").strValue());
			dbrxx.setHaveGlobalIdCopy(dbrarray.getStruct(i).getField("HARDCOY_FLAG").strValue());
			dbrxx.setPhone_1(dbrarray.getStruct(i).getField("MOBILE").strValue());
			dbrxx.setPhone_2(dbrarray.getStruct(i).getField("MOBILE2").strValue());
			dbrxx.setPhoneOther(dbrarray.getStruct(i).getField("MOBILE3").strValue());
			dbrxx.setWorkUnitName(dbrarray.getStruct(i).getField("COMMPANY_NAME").strValue());
			dbrxx.setWorkYear(dbrarray.getStruct(i).getField("WORK_YEAR").strValue());
			dbrxx.setWorkUnitAddr(dbrarray.getStruct(i).getField("COMMPANY_ADDR").strValue());
			dbrxx.setWorkUnitPhone(dbrarray.getStruct(i).getField("COMMPANY_PHONE").strValue());
			//TODO xiegl
			dbrxx.setYeahIncome(dbrarray.getStruct(i).getField("PERSON_ANNUAL_INCOME").doubleValue()+"");
			dbrxx.setHaveProveFile(dbrarray.getStruct(i).getField("FILE_FLAG").strValue());
			dbrxx.setFileName(dbrarray.getStruct(i).getField("FILES_NAME").strValue());
			dbrxx.setMarriage(dbrarray.getStruct(i).getField("MARITAL_STATUS").strValue());
			dbrxx.setHomePhone(dbrarray.getStruct(i).getField("HOUSE_PHONE").strValue());
			dbrxx.setHomeAddr(dbrarray.getStruct(i).getField("HOME_ADDR").strValue());
			dbrxx.setHomeType(dbrarray.getStruct(i).getField("HOME_TYPE").strValue());
			dbrList.add(dbrxx);
			...
			for(int j =0;j<loanArray.size();j++){
				if(!SEQU_NO.equals(loanArray.getStruct(j).getField("SEQU_NO").strValue())){
					continue;
				}
				QzApplnDbrxxDkjl dkjl = new QzApplnDbrxxDkjl();
				dkjl.setDbrxxId(dbrxx.getId());
				dkjl.setLsh(j+1);
				dkjl.setLoanType(loanArray.getStruct(j).getField("FUND_SOURCE").strValue());
				dkjl.setAmount(loanArray.getStruct(j).getField("LOAN_AMT").strValue());
				dkjl.setDeadline(loanArray.getStruct(j).getField("TERM").strValue());
				dkjl.setLoanDate(StringUtils.isEmpty(loanArray.getStruct(j).getField("DRAW_DOWN_DATE").strValue())?new Date():formatter10.parse(loanArray.getStruct(j).getField("DRAW_DOWN_DATE").strValue()));
				dkjl.setPurpose(loanArray.getStruct(j).getField("USAGE").strValue());
				dkjl.setRemainAmount(loanArray.getStruct(j).getField("LOAN_BALANCE").strValue());
				dkjl.setGuntForOther(loanArray.getStruct(j).getField("GNT_FLAG").strValue());
				dkjl.setGuntAmount(loanArray.getStruct(j).getField("GNT_AMT").strValue());
				dkjl.setGuntDeadline(loanArray.getStruct(j).getField("GNT_TERM").strValue());
			...
			for(int j =0;j<fc.size();j++){
				if(!SEQU_NO.equals(fc.getStruct(j).getField("SEQU_NO").strValue())){
					continue;
				}
				QzApplnDbrxxFc fcobj = new QzApplnDbrxxFc();
				fcobj.setDbrxxId(dbrxx.getId());
				fcobj.setLsh(j+1);
				fcobj.setAddr(fc.getStruct(j).getField("ADDRESS").strValue());
				fcobj.setPrice(fc.getStruct(j).getField("COST").strValue());
				fcobj.setUsrSituation(fc.getStruct(j).getField("USER_INFO").strValue());
				fcobj.setHaveCopy(fc.getStruct(j).getField("HARDCOY_FLAG").strValue());
				
				commonDao.insertObject(fcobj);
			}
			//机动车信息数组
			Array jdcarray = BODY.getArray("VEHICLE_INFO_ARRAY");
			for(int j =0;j<jdcarray.size();j++){
				if(!SEQU_NO.equals(jdcarray.getStruct(j).getField("SEQU_NO").strValue())){
					continue;
				}
				QzApplnDbrxxJdc jdc = new QzApplnDbrxxJdc();
				jdc.setDbrxxId(dbrxx.getId());
				jdc.setLsh(j+1);
				jdc.setCardNo(jdcarray.getStruct(j).getField("VEH_NO").strValue());
				jdc.setPrice(jdcarray.getStruct(j).getField("PRICE").strValue());
				jdc.setBuyDate(StringUtils.isEmpty(jdcarray.getStruct(j).getField("BUY_DATE").strValue())?new Date():formatter10.parse(jdcarray.getStruct(j).getField("BUY_DATE").strValue()));
				jdc.setHaveCopy_1(jdcarray.getStruct(j).getField("VEHICLE_HARDCOY_FLAG").strValue());
				jdc.setHaveCopy_2(jdcarray.getStruct(j).getField("HARDCOY_FLAG").strValue());
		...
		qz.setUnitTypeOther(BODY.getField("REMARK4").strValue());
		qz.setBussdistrictAddress(BODY.getField("ADDRESS").strValue());
		
		//待上传资料
		QzApplnAttachmentList attach = null;
		attach = attachmentListService.findAttachmentListByAppId(INPUT_WARE_REG_NO);
		if(attach == null){
			attach = new QzApplnAttachmentList();
		}
		...
		if(qz.getId() == null){
			...
			commonDao.updateObject(qz);
		}
		//供应商数组
		//删除供应商数组重新插入
		sql = "delete from qz_appln_ywsqb_zygys where ywsqb_id = '"+qz.getId()+"'";
		commonDao.queryBySql(sql, null);
		for(QzApplnYwsqbZygys gys :gysList){
			gys.setYwsqbId(qz.getId());
			commonDao.insertObject(gys);
		}
		//删除客户数组重新插入
		sql = "delete from qz_appln_ywsqb_zykh where ywsqb_id = '"+qz.getId()+"'";
		commonDao.queryBySql(sql, null);
		for(QzApplnYwsqbZykh khs :khList){
			khs.setYwsqbId(qz.getId());
			commonDao.insertObject(khs);
		}
		//删除贷款信息明细数组重新插入
		sql = "delete from qz_appln_ywsqb_Jkjl where ywsqb_id = '"+qz.getId()+"'";
		commonDao.queryBySql(sql, null);
		for(QzApplnYwsqbJkjl jkjl:jkjlList){
			jkjl.setYwsqbId(qz.getId());
		...
		if(attach.getId() == null){
			...
			commonDao.updateObject(attach);
		}
		
		//图片信息明细数组
		Array pic_arr = BODY.getArray("PICTURE_INFO_ARRAY");
		attachmentListService.addBatchInfo_PAD(INPUT_WARE_REG_NO,pic_arr);
		return null;
	}
	...
	public CompositeData ipadWdAuditInfo(CompositeData BODY) throws ParseException, IpadException {
		...
		for(ApproveHistoryForm obj : ls ){
			...
			Field APPROVE_DATE = new Field(new FieldAttr(FieldType.FIELD_STRING, 30));
			APPROVE_DATE.setValue(formatter14.format(obj.getStartExamineTime()));
			struct.addField("APPROVE_DATE", APPROVE_DATE);
			
			Field APPROVE_LIMIT_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	...
	//ipad 客户用信查询
    public CompositeData ipadKhyxInfo(CompositeData BODY,String limit,String page) throws ParseException, IpadException {
    	//客户代码
        ...
        if(cont != null){
        	Field CREIDT_LIMIT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
        ...
        DictionaryManager dictMgr = Beans.get(DictionaryManager.class);
		Dictionary dictionary = dictMgr.getDictionaryByName("LOAN_STATUS");//FIELD_NAME[i]具体数据字典字段
		
//		int start = (Integer.parseInt(page)-1)*Integer.parseInt(limit)+1;
//		int end = Integer.parseInt(page)*Integer.parseInt(limit);
		//TODO xiegl
		int start = Integer.parseInt(page)*Integer.parseInt(limit)+1;
		int end = (Integer.parseInt(page) + 1)*Integer.parseInt(limit);
      	...
      	for(int i=0;i<loanList.size();i++){
			...
			Field CLIENT_CODE = new Field(new FieldAttr(FieldType.FIELD_STRING, 16));
			CLIENT_CODE.setValue(loanList.get(i).getCustomerId());
			struct.addField("CLIENT_CODE", CLIENT_CODE);
			
			Field DUEBILL_NO = new Field(new FieldAttr(FieldType.FIELD_STRING, 50));
			DUEBILL_NO.setValue(loanList.get(i).getBillNo());
			struct.addField("DUEBILL_NO", DUEBILL_NO);
			
			Field CONTRACT_NO = new Field(new FieldAttr(FieldType.FIELD_STRING, 30));
			CONTRACT_NO.setValue(loanList.get(i).getContNo());
			struct.addField("CONTRACT_NO", CONTRACT_NO);
			
			Field LOAN_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
			LOAN_AMT.setValue(loanList.get(i).getLoanAmt());
			struct.addField("LOAN_AMT", LOAN_AMT);
			
			Field DRAWDOWN_DATE = new Field(new FieldAttr(FieldType.FIELD_STRING, 8));
			DRAWDOWN_DATE.setValue(loanList.get(i).getDistrDate());
			struct.addField("DRAWDOWN_DATE", DRAWDOWN_DATE);
			
			Field OVERDUE_TERMS = new Field(new FieldAttr(FieldType.FIELD_STRING, 16));
			OVERDUE_TERMS.setValue(loanList.get(i).getOverdue()+"");
			struct.addField("OVERDUE_TERMS", OVERDUE_TERMS);
			
			Field OVERDUE_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
			OVERDUE_AMT.setValue(loanList.get(i).getOverdueBalance());
			struct.addField("OVERDUE_AMT", OVERDUE_AMT);

			Field BOOK_STATUS = new Field(new FieldAttr(FieldType.FIELD_STRING, 10));
			List<DictionaryItem> dictItems = dictionary.getItems();
			for(DictionaryItem item : dictItems){
				if(item.getName().equals(loanList.get(i).getAccStatus())){
					BOOK_STATUS.setValue(item.getTitle());
					break;
				}
			}
			...
			if(loanList.get(i).getAccStatus().equals("1")){
				usedLimit += Double.valueOf(loanList.get(i).getLoanBalance());
			}
      	}
      	
      	Field USED_LIMIT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
    ...
    public CompositeData ipadDhJcAddInfo(CompositeData BODY) throws ParseException, IpadException {
    	...
    	String  cus_id = BODY.getField("CLIENT_NO").strValue();
    	String  clientCode = BODY.getField("CLIENT_CODE").strValue();
    	String  taskType = BODY.getField("TASK_TYPE").strValue();
    	String  taskDate = BODY.getField("TASK_DATE").strValue();
    	double  loanAmt = BODY.getField("LOAN_AMT").doubleValue();
    	double  loanBalance = BODY.getField("LOAN_BALANCE").doubleValue();
    	...
    	psptask.setCusId(cus_id);
    	psptask.setTaskType(taskType);
    	psptask.setTaskCreateDate(taskDate);
    	psptask.setLoanTotlAmt(loanAmt);
    	psptask.setLoanBalance(loanBalance);
        ...
        psptask.setIndustryOutlook(riskLevel);
        ...
        //图片信息明细数组
  		...
  		for(int i =0;i<pic_arr.size();i++){
  			String tmp_sql = "insert into qz_appln_attachment_detail (id,BATCH_ID,ORIGINAL_NAME,FILE_NAME,IS_UPLOAD)values("
  					+ "'"+pic_arr.getStruct(i).getField("PICTURE_CACHE_NAME").strValue().split("\\.")[0]+"',"
  							+ "'"+taskId+cus_id+"',"
  									+ "'"+pic_arr.getStruct(i).getField("PICTURE_NAME").strValue()+"',"
  											+ "'"+pic_arr.getStruct(i).getField("PICTURE_CACHE_NAME").strValue()+"',"
  													+ "'1')";
  			commonDao.queryBySql(tmp_sql, null);
  		}
  		
      		
        String sql = "update psp_check_task SET APPROVE_STATUS = '1',upload_flag = '1',agreed_person='"+psptask.getAgreedPerson()+"'"
				+ ", check_addr='"+psptask.getCheckAddr()+"',  check_time='"+psptask.getCheckTime()+"'"
				+ ", remarks='"+psptask.getRemarks()+"', industry_outlook='"+psptask.getIndustryOutlook()+"', repayment='"+psptask.getRepayment()+"'"
				+ ", reciprocal_type='"+psptask.getReciprocalType()+"', contact_information='"+psptask.getContactInformation()+"',repayment_other='"+psptask.getRepaymentOther()+"' "
				+ " where task_id='"+psptask.getTaskId()+"'";
        commonDao.queryBySql(sql, null);
    	return null;
    }
    ...
    public CompositeData ipadDhJcSearchInfo(CompositeData BODY) throws ParseException, IpadException {
        ...
        if(vo !=null){
	      	...
	      	Field CLIENT_CODE = new Field(new FieldAttr(FieldType.FIELD_STRING, 16));
	      	CLIENT_CODE.setValue(vo.getCustomerId());
	      	respBody.addField("CLIENT_CODE",CLIENT_CODE);
	      	
	      	Field CLIENT_NO = new Field(new FieldAttr(FieldType.FIELD_STRING, 16));
	      	CLIENT_NO.setValue(vo.getCusId());
	      	respBody.addField("CLIENT_NO",CLIENT_NO);
	      	
	      	Field TASK_TYPE = new Field(new FieldAttr(FieldType.FIELD_STRING, 100));
			if(StringUtils.isNotEmpty(vo.getCheckDescribe())){
				TASK_TYPE.setValue(vo.getCheckDescribe());
			}
			else{
				if(vo.getTaskType().equals("1")){
					TASK_TYPE.setValue("首次放款检查");
				}else if(vo.getTaskType().equals("2")){
					TASK_TYPE.setValue("满额用信");
				}else if(vo.getTaskType().equals("3")){
					TASK_TYPE.setValue("授信到期前最高额用信检查");
				}else if(vo.getTaskType().equals("4")){
					TASK_TYPE.setValue("放款次月客户回访");
				}else if(vo.getTaskType().equals("5")){
					TASK_TYPE.setValue("每月电话回访");
				}else if(vo.getTaskType().equals("4")){
					TASK_TYPE.setValue("季度回访");
				}
	    	...
	    	CHECK_POSITION.setValue(vo.getCheckAddr());
	      	...
	      	RISK_LEVEL.setValue(vo.getIndustryOutlook());
    ...
    public CompositeData ipadZaInfo(CompositeData BODY) throws ParseException, IpadException {
    	//返回结果
    	List<QzApplnZa> zaList  = customerInforDao.ipadApplyZaListApproved();
    	CompositeData respBody = new CompositeData();
     	...
     	Array array = new Array();
       	for(QzApplnZa za :zaList){
      		...
      		Field PROJECT_ID = new Field(new FieldAttr(FieldType.FIELD_STRING, 20));
      		PROJECT_ID.setValue(za.getId());
			struct.addField("PROJECT_ID", PROJECT_ID);
			...
			Field PROJECT_CODE = new Field(new FieldAttr(FieldType.FIELD_STRING, 20));
			PROJECT_CODE.setValue(za.getCode());
			struct.addField("PROJECT_CODE", PROJECT_CODE);
    ...
    public CompositeData ipadWdspInfoList(CompositeData BODY) throws ParseException, IpadException {
    	...
    	//查询进件信息
    	List<CustomerApplyInfoIpad> tmp = customerInforDao.Wdsqspbrowse(INPUT_WARE_REG_NO,CLIENT_CODE);
    	if(tmp == null || tmp.size()==0){
    		return null;
    	}
    	CustomerApplyInfoIpad applyInfo = tmp.get(0) ;
    	
		if(applyInfo.getStatus().equals(Constant.SAVE_INTOPICES_CN)){
			applyInfo.setProcess("未提交申请");
		} else if(applyInfo.getStatus().equals(Constant.APPROVE_INTOPICES_CN)||applyInfo.getStatus().equals(Constant.TRTURN_INTOPICES_CN)){
		...
		} else if(applyInfo.getStatus().equals(Constant.RETURN_INTOPICES_CN)){
        ...
        //审批信息数组
      	List<ApproveHistoryForm> ls = customerInforDao.ipadWdAuditInfo(applyInfo.getInputWareRegNo());
		...
		for(ApproveHistoryForm obj : ls ){
			...
			Field APPROVE_LIMIT_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
			APPROVE_LIMIT_AMT.setValue(obj.getExamineAmount()==null?Double.valueOf("0.0"):Double.valueOf(obj.getExamineAmount())/100);
			struct.addField("APPROVE_LIMIT_AMT", APPROVE_LIMIT_AMT);
    ...
    public CompositeData ipadWdApplyInfoList(CompositeData BODY) throws ParseException, IpadException {
    	...
    	if(qz!=null){
	    	...
	    	USER_NO.setValue(ecif.getUserId());
	      	respBody.addField("USER_NO", USER_NO);
	      	
			Field CLIENT_CODE = new Field(new FieldAttr(FieldType.FIELD_STRING, 50));
	      	CLIENT_CODE.setValue(appInfo.getCustomerId());
	      	respBody.addField("CLIENT_CODE", CLIENT_CODE);
	    	...
	    	LOAN_PURPUSE_CODE.setValue(qz.getApplyUse());
	      	...
	      	Field MONTH_REPAY = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	      	...
	      	Field LICENSE_INCOME = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	      	...
	      	Field PROFIT_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	      	...
	      	Field ASSET_TOTAL_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	      	...
	      	Field OUGHT_ACCEPT_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	    	...
	    	Field OUGHT_PAY_AMT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	    	OUGHT_PAY_AMT.setValue(qz.getOutMoney()==null?Double.valueOf("0.0"):Double.valueOf(qz.getOutMoney()));
	      	...
	      	Field LIBILITY = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	      	...
	      	Field OTHER_INCOME = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	      ...
	      //贷款申请表
			...
			PLEDGE_FLAG.setValue(qz.getHaveGunt());
	      	...
	      	respBody.addField("LOAN_USER4",LOAN_USER4);
	    	...
	    	GNT_OTHER_BANK.setValue(qz.getGuntForOtherBankname());
	      	...
	      	if(qz.getHaveApplyLoan().equals("0")){
	      		DR_DATE.setValue("");
	      	}else{
	    	...
	    	APP_NO.setValue(qz.getManagerId());
	      	...
	      	Field HELPERS = new Field(new FieldAttr(FieldType.FIELD_STRING,150));
	      	HELPERS.setValue(qz.getHelpManagerName());
	      	respBody.addField("HELPERS",HELPERS);
	    	...
	    	WORK_YEAR.setValue(qz.getWorkYear());
	     	...
	     	CORP_ORG_FORM.setValue(qz.getUnitType());
	      	...
	      	Field UNIT_TYPE_OTHER = new Field(new FieldAttr(FieldType.FIELD_STRING,50));
	      	UNIT_TYPE_OTHER.setValue(qz.getUnitTypeOther());
	      	respBody.addField("REMARK4",UNIT_TYPE_OTHER);
			...
			LOAN_FLAG.setValue(qz.getBorrowHistory());
			respBody.addField("LOAN_FLAG", LOAN_FLAG);
    	}
    	
      	//经营信息
    	
      	QzApplnJyxx jyxx = jyxxService.findJyxx(appInfo.getCustomerId(),null);
      	if(jyxx!= null){
	    	...
	    	BSN_ADDRESS_TYPE.setValue(jyxx.getBussAddrType());
	      	...
	      	EMPLOYEE_NUMBER.setValue(jyxx.getBussEmployeeNum()==null?0:Integer.parseInt(jyxx.getBussEmployeeNum()));
		...
		for(QzApplnYwsqbZygys gy :gysList){
			...
			Field LOT = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
		...
		for(QzApplnYwsqbJkjl jkjl :jkjlList){
			...
			Field AMT = new Field(new FieldAttr(FieldType.FIELD_STRING, 50));
			AMT.setValue(jkjl.getTotalAmount());
			...
			Field INT_RATE = new Field(new FieldAttr(FieldType.FIELD_STRING,50));
			INT_RATE.setValue(jkjl.getRates());
			...
			Field LOAN_BALANCE = new Field(new FieldAttr(FieldType.FIELD_STRING, 50));
			LOAN_BALANCE.setValue(jkjl.getRemainSum());
			struct.addField("LOAN_BALANCE", LOAN_BALANCE);
		...
		for(QzApplnDbrxx dbrxx :dbrList){
			...
			struct.addField("COMMPANY_PHONE",COMMPANY_PHONE);
			
			Field PERSON_ANNUAL_INCOME = new Field(new FieldAttr(FieldType.FIELD_DOUBLE, 20, 2));
	    ...
	    for(QzApplnDbrxx dbrxx :dbrList){
			...
			for(QzApplnDbrxxDkjl dk:dkjList){
				...
				Field loanAmt = new Field(new FieldAttr(FieldType.FIELD_STRING,50));
				loanAmt.setValue(dk.getAmount());
				...
				Field LOAN_BALANCE = new Field(new FieldAttr(FieldType.FIELD_STRING,50));
				LOAN_BALANCE.setValue(dk.getRemainAmount());
				struct.addField("LOAN_BALANCE",LOAN_BALANCE);
				...
				Field GNT_AMT = new Field(new FieldAttr(FieldType.FIELD_STRING,50));
				GNT_AMT.setValue(dk.getGuntAmount());
				struct.addField("GNT_AMT",GNT_AMT);
				
				Field GNT_TERM = new Field(new FieldAttr(FieldType.FIELD_STRING,20));
				GNT_TERM.setValue(dk.getGuntDeadline());
			...
			//房产
			List<QzApplnDbrxxFc> fcList = customerInforDao.findDbrxxFc(dbrxx.getId());
			for(QzApplnDbrxxFc f :fcList){
				...
				Field COST = new Field(new FieldAttr(FieldType.FIELD_STRING,50));
				COST.setValue(f.getPrice());
			...
			//机动车
			List<QzApplnDbrxxJdc> jdcList = customerInforDao.findDbrxxJdc(dbrxx.getId());
			for(QzApplnDbrxxJdc jdc :jdcList){
				...
				Field PRICE = new Field(new FieldAttr(FieldType.FIELD_STRING,50));
				PRICE.setValue(jdc.getPrice());
		...
		//待上传资料
      	...
      	if(attach!=null){
        	...
        	BUSS_TYPE.setValue(attach.getBussType());
      		...
      		attach = new QzApplnAttachmentList();
      		attach.setApplicationId(INPUT_WARE_REG_NO);
      		attach.setDocid(formatter17.format(new Date()));
      		commonDao.insertObject(attach);
    ...
    public CompositeData getEmptyRes(CompositeData BODY) throws IpadException{
    	//进件登记号
    	String  INPUT_WARE_REG_NO = BODY.getField("INPUT_WARE_REG_NO").strValue();
    	//验证
    	if(StringUtils.isEmpty(INPUT_WARE_REG_NO)){throw new IpadException("进件登记号必输!");}
		...
		Field APP_ID = new Field(new FieldAttr(FieldType.FIELD_STRING, 20));
    	APP_ID.setValue(appInfo.getId());
      	respBody.addField("INPUT_WARE_REG_NO", APP_ID);
      	
      	Field CLIENT_CODE = new Field(new FieldAttr(FieldType.FIELD_STRING, 50));
      	CLIENT_CODE.setValue(ecif.getCustomerId());
      	respBody.addField("CLIENT_CODE", CLIENT_CODE);
    	...
    	SEX.setValue(ecif.getSex());
      	...
      	Field FLAG = new Field(new FieldAttr(FieldType.FIELD_STRING,1));
      	FLAG.setValue(formatter17.format(new Date()));
      	respBody.addField("FLAG",FLAG);
      	
      	User user = userService.getUserById(ecif.getUserId());
      	UserIpad userIpad = this.findInfoByLogin(user.getLogin());
      	Field APP_BRANCH_NAME = new Field(new FieldAttr(FieldType.FIELD_STRING, 100));
      	APP_BRANCH_NAME.setValue(userIpad.getOrgName());
      	respBody.addField("APP_BRANCH_NAME", APP_BRANCH_NAME);
      	
    	Field APP_BRANCH_ID = new Field(new FieldAttr(FieldType.FIELD_STRING, 20));
    	APP_BRANCH_ID.setValue(userIpad.getOrgId());
      	respBody.addField("APP_BRANCH_ID", APP_BRANCH_ID);
		...
		Field EDUCATION = new Field(new FieldAttr(FieldType.FIELD_STRING, 1));
      	EDUCATION.setValue(ecif.getEducationLevel());
      	respBody.addField("EDUCATION",EDUCATION);
      	
      	Field MOBILE = new Field(new FieldAttr(FieldType.FIELD_STRING, 30));
      	MOBILE.setValue(ecif.getContactMode());
      	respBody.addField("MOBILE",MOBILE);
      	
      	Field ADDRESS = new Field(new FieldAttr(FieldType.FIELD_STRING,300));
      	QzApplnZa za = customerInforDao.findZaByAppId(INPUT_WARE_REG_NO);
      	if(za != null){
      		ADDRESS.setValue(za.getAddress());
      	}
      	else{
      		ADDRESS.setValue("");
      	}
      	...
      	QzApplnJyxx jyxx = null;
		jyxx = jyxxService.findJyxx(appInfo.getCustomerId(),null);
		
		if(jyxx != null){
			Field CMPNY_NAME = new Field(new FieldAttr(FieldType.FIELD_STRING,150));
	      	CMPNY_NAME.setValue(jyxx.getBussName());
	      	respBody.addField("CMPNY_NAME",CMPNY_NAME);
		}
