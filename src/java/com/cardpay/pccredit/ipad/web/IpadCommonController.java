	...
	public CompositeData exec(CompositeData cd) {
		...
		try {
			...
			//0200300000265微贷客户用信查询
				...
				respCd = this.getRespCdCommon("成功",IpadConstant.RET_CODE_SUCCESS,ipadCommonService.ipadKhyxInfo(BODY,limit,page));
			}
			...
			//0200300001202微贷进件信息查询
				...
				CompositeData tmp = ipadCommonService.ipadWdApplyInfoList(BODY);
				if(tmp == null){
					respCd = this.getRespCdCommon("查无结果",IpadConstant.RET_CODE_SUCCESS,ipadCommonService.getEmptyRes(BODY));
				}
				else{
					respCd = this.getRespCdCommon("成功",IpadConstant.RET_CODE_SUCCESS,tmp);
				}
			...
			//0200300001403微贷申请审批查询
				...
				CompositeData tmp = ipadCommonService.ipadWdspInfoList(BODY);
				if(tmp == null){
					respCd = this.getRespCdCommon("该客户无进件",IpadConstant.RET_CODE_EMPTY,null);
				}
				else{
					respCd = this.getRespCdCommon("成功",IpadConstant.RET_CODE_SUCCESS,tmp);
				}
